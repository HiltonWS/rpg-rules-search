...
from rpg_rules_search.database import connect, initialize, replace_image_tags
from rpg_rules_search.ollama import DEFAULT_OLLAMA_MODEL, OllamaClient, suggest_image_tags


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reprocessa tags de todas as imagens com IA local.",
    )
    parser.add_argument(
        "--database",
        type=Path,
    ...
    parser.add_argument("--limit", type=int, default=500, help="Número máximo de imagens.")
...
def main() -> int:
    ...
    for row in rows:
        try:
            ...
            replace_image_tags(connection, int(row["id"]), tags, source="ai-retag")
            tagged += 1
    ...
    print(f"Concluído. Processadas: {len(rows)} | Re-tagueadas: {tagged}")
    connection.close()
    return 0
